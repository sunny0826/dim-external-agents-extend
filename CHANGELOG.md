...
- **`show-external-agents` skill** — a slash-menu entry that does exactly one thing: open the panel. Picking it calls `open_agent_run_log` once and confirms, with no follow-up questions and no queries — it deliberately does *not* call `list_agent_runs` / `read_agent_run` (that is what the `external-agents-extend` skill is for) and does not show the inline card (`show_external_agents` is "look at it in the conversation", not "open the panel"). Use it when you just want the panel up
- **Agent name in the panel** — the task list and the log page header now show the name dim gave the agent (`agentName`, e.g. `林澈`), read from the task's `metadata.agentName` (present on all 314 agent tasks on this machine). It renders inside the agent badge on the list page (`[logo] CODEX | 林澈`, deliberately exempt from the badge's uppercase / letter-spacing / monospace treatment so CJK names stay readable) and as its own element in the log header, with a tooltip naming the source. Runs without a name render exactly as before — the log-header element is hidden instead of left as an empty flex item, so the 10px gap does not double. No backend change was needed: `summarizeRun` already carried the field through `list_agent_runs` and `read_agent_run`
