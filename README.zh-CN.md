...
- **任务列表** — 列出本会话（或全部历史）委托给外部 Agent 的任务：Agent 类型**以及 dim 给它起的名字**（`agentName`，如「林澈」）、模型、状态、开始时间与耗时
...
也可以从输入区的「技能」按钮或斜杠菜单选择 `/show-external-agents` 直接打开面板、`/external-agents-extend` 查询某个任务的执行情况，或选择 `/external-session-names` 整理会话名称。

### 全屏日志面板

![日志页](docs/images/panel-log.png)

- 顶部是任务上下文：对应 Agent 的 logo、Agent 类型、**dim 给它起的名字**（`agentName`）、模型、标题、状态与时间范围
...
**任务显示「没有可读日志」？**
三种情况：① 该任务对应的外部会话已被清理（超出 Agent 自身的会话保留期）；② 该 Agent 类型的日志读取尚未支持（ZCode）；③ 会话文件还在，但创建时间落在匹配窗口之外——插件用一段按 Agent 类型取 5–30 秒的窗口把任务关联到会话文件。
