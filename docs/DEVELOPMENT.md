...
- 2026-09-19 三次增补（会话名称统一，v0.0.3）：新增 `list_external_sessions` / `rename_external_sessions` 两个 MCP 工具与 `sessions` / `rename` 两个 CLI 子命令，外加 skill `external-session-names`。把各外部 agent 自己攒的会话名统一为 `[codex] 09-19 20:31 · 修复 GUO-108 审查问题`：命名优先级 = dim 任务标题 → 会话自身标题（通用标题如 `Help` / `New session - <ISO>` / 纯 uuid 被识别并跳过）→ 首条 prompt 里有意义的行（跳过委托包装样板，含 Issue token 的行优先）→ `未命名会话 · <cwd 末段或会话短码>`。回写（默认 dry-run、写前备份到 `~/.dimcode/ea-extend-backups/`、幂等、保护用户自定义标题）覆盖全部 6 个 agent：codex（`session_index.jsonl.thread_name`）/ kimi（`state.json.title` + `isCustomTitle`）/ cursor（`meta.json.title`）/ grok（`summary.json` 的 `generated_title` + `title_is_manual`，**不写** `session_search.sqlite`——那只是会重建的搜索索引）/ opencode（`session.title`，大库只备份受影响行）/ zcode（`~/.zcode/cli/db/db.sqlite` 的 `session.title` + `title_source='custom'`；桌面 `tasks-index.sqlite` 是任务索引不是会话表，仅展示）。**修掉一处根因级错配**：zcode 适配器原先读桌面任务索引（0 行），真实会话在 CLI 库。测试 159 项全绿
- 2026-09-19 四次增补（来源标记 + 全自动命名）：① 统一名带来源标记——`[dim]` 表示由 dim 委托产生、`[手动]` 表示其它来源（展示名形如 `[codex] 09-19 20:31 · [dim] 修复 GUO-108 审查问题`），工具输出同时给 `source` 与 `delegated`（后者识别「被某个编排器拉起但不是 dim」的情况），`sourcePrefix: false` 可关闭标记；② 新增 `hooks/auto-name.js`（挂在 UserPromptSubmit / PostToolUse / Stop）：dim 委托后**自动**把该外部会话的泛化标题改写成统一名，无需对话或 CLI——只动能关联到 dim 任务、且自身标题泛化的会话，只回溯 2 小时，写前备份，每任务一次，10 秒节流，静默；`EA_EXT_AUTO_NAME=off` 或 `~/.dimcode/ea-extend-config.json` 的 `{autoName:false}` 可关闭；③ 新增 `auto_name_sessions` 工具与 `autoname` 子命令用于显式回填（默认 dry-run）；④ 名称识别增强：只剩委托前缀的标题（「你是实现者」）算泛化，带任务信息的先剥前缀（「你是 Project V 的执行开发者。任务：时间轴重构」→「任务：时间轴重构」），并修掉「二次改名叠加标记成 `[手动] [手动] …`」的缺陷；⑤ 自动命名改为**默认关闭（opt-in）**：开关由 `~/.dimcode/ea-extend-config.json` 的 `autoName` 决定（环境变量 `EA_EXT_AUTO_NAME` 优先，默认关闭），`autoname --enable/--disable` 一条命令切换；⑥ 开关可在桌面端操作：全屏面板右上角新增「自动命名」开关（widget 经新增的 `get_settings` / `set_auto_name` 工具读写配置），也可在对话里说一句；dim 原生插件设置页只服务内置插件（读 `settingsSchema` + `controller`），第三方插件只能自带开关；⑦ 修复面板开关位置：自动命名开关原先复用 `label.follow` 类，而 `body.view-list label.follow{display:none}` 把该类限定为「仅日志详情页显示」——导致一个**全局设置**反而只在任务详情页可见、列表页看不到；改用独立的 `label.global` 类（列表页右上角，详情页与 inline 卡片隐藏），并加测试锁住该意图（197 项）；⑧ 修复「并发会话下运行中的外部 Agent 看似没被识别」：`scope=session` 原先依赖一个全局单会话文件（任何会话的 hook 都会覆盖它），两个会话同时在用时面板会解析到别的会话而显示空列表——改为按会话记录最近活跃时间的映射（`server/src/core/active-session.js`，hook 侧转发），并在「解析到的会话没有任务、且其它会话仍有正在运行的任务」时回退为全部会话 + `scopeFallback` 说明（含在跑数量）+ 每条任务带 `sessionId`（别处只剩历史时保持空态，避免默认视图倒出历史失败），`list_agent_runs` 支持显式 `sessionId`；⑨ 面板交互整理（2026-09-20）：列表页筛选收敛为一个「筛选」按钮 + 弹出面板（全部会话 / 显示已结束 / **显示失败**，工具层新增 `includeFailed`，默认只看本会话且隐藏已结束与失败）；筛选与「自动命名」开关改为**仅列表页**（日志详情页不再显示列表级控件）；自动命名改为开关样式并加 hover 提示（文案如实写明「立即生效、无需重启」——hook 每次事件新起进程并即时读配置）；日志详情页头部加对应 Agent 的 logo（复用列表页同一套内联图标）。测试 201 项全绿；浏览器实测：开关未选/选中轨道与滑块、hover 提示可见、筛选开合与角标计数、列表页/详情页控件可见性、logo 样式规则；⑩ 顶栏对齐修正（2026-09-20）：`.switch-wrap`/`.filter` 这类"包裹一个 inline-flex 子元素"的容器必须是 `display: flex; align-items: center`——若留默认 `block`，子元素按基线排版会贴在行顶，底部留下 strut 的下伸空间，整组比相邻控件（标题/徽标/刷新）高出约 2px。已加测试断言这两条规则含 `display: flex` + `align-items: center`；浏览器实测各元素中心与顶栏中心偏差一致（均为 -0.5px）。⑫ 三项增补（2026-09-20）：①**grok 日志打通**——`mapping.js` 新增 `findGrokRef`（按 `~/.grok/sessions/<enc-cwd>/<id>/summary.json` 的 `created_at` 定位，实测比任务派发晚 0.1–2.8s，5s 窗口足够；消歧顺序为 **prompt 指纹 > Issue ID token > 时间差**：dim 把任务 prompt 原样作为首条 user 消息交给 CLI，读 `updates.jsonl` 头部取首条 `user_message_chunk` 与 `run.prompt` 的归一化首/尾 60 字窗口比对——实测两个并行 grok 会话创建时间只差 94ms 且标题完全相同，只有 prompt 能区分，纯 delta 会把「审查 PR #53」的任务显示成「实现 M2-04b」的日志），新增 `server/src/core/adapters/grok.js` 把 `updates.jsonl`（ACP 风格 session/update 流，每行带 `timestamp` 秒 / `_meta.agentTimestampMs` 毫秒）归一化为统一事件：`hook_execution`/`user_message_chunk`/`plan`/`retry_state`/`task_backgrounded`/`task_completed`/`background_tasks` → notice，`agent_thought_chunk` → think，`agent_message_chunk` → text，`tool_call` → tool_call（detail 带 rawInput 与工具元信息），`tool_call_update`（有 status）→ tool_result（按 toolCallId 关联工具名），无 status 的补充描述不产事件只计数，`turn_completed` → usage（模型名兜底来源）；cursor = updates.jsonl 字节偏移，尾部半行不消费；②**筛选覆盖全部状态**——面板筛选改为四个状态复选框（运行中/已完成/已取消/失败，可多选）+ 范围（本会话/全部会话），工具层新增 `statuses`（空数组 = 不按状态筛，避免全部取消勾选时静默空列表），默认仍只看本会话的运行中任务；③**「跟随」改开关样式**——复用 `label.switch` 的轨道+滑块，作用域规则从 `label.follow` 改为 `#followSwitch`（列表页与 inline 卡片隐藏）。测试 213 项全绿；本机 8 个 grok 任务全部 high 置信度定位；浏览器实测：菜单五项（状态 4 + 范围 1）与分组标题/分隔线、角标 0→1→2→3、标题文案（中文状态名）、列表页/日志页控件可见性、跟随开关轨道 26×15 且与返回按钮中心一致。（kimi state.json / cursor meta.json / grok summary.json 原本是压缩单行，不再被改写成美化格式），并新增 `restore` 子命令与 `restore_backups` 工具按备份字节回滚。测试 201 项全绿
- 2026-09-20 五次增补（opencode 日志适配）：`mapping.js` 新增 `findOpencodeRef`、新增 `server/src/core/adapters/opencode.js` 并注册进适配器分发表，opencode 任务不再返回「暂不支持 opencode 的会话定位」。**根因级差异**：opencode 的库是**状态式**存储而非追加式日志——part 行会被**原地改写**（tool 从 `state.status='running'` 改写成 `'completed'`/`'error'`，实测某会话 74/74 个 tool part 的 `time_updated ≠ time_created`），text/reasoning 更是**流式写入**（实测 time_updated − time_created：text 平均 3.4s / 最长 35s，reasoning 平均 5.2s / 最长 83s）。于是字节偏移游标不存在，而「按 time_created 单调推进」会让先落库、后完成的 part 永远停在中间态（工具永远显示「调用中」、文本被截断）。解法是用 opencode 自己的 **step 结构**作**稳定边界**——`step-start` … `step-finish` 是瞬时写入（实测差值 ≤ 5ms），一条 `step-finish` 落库即表示该 step 内的 part 已全部写完，因此**只输出位置 ≤ 最后一条 step 标记（step-start 或 step-finish）的 part**，进行中 step 的内容留给下次；取「最后一条 step 标记」而非「最后一条 step-finish」，是为了让进行中 step 的 `step-start` 立刻可见（否则运行中的任务在日志里看起来完全停滞）。游标为 (time_created, id) 复合位置（id 是 ULID 风格、与创建时间同序），每条 part 只在终态输出一次，天然不重复。事件映射：user message 的 text → notice(user_message)、assistant text → text、reasoning → think、tool → tool_call + tool_result（同 callID 配对，running 时只给 tool_call 并给 `tool_in_flight` 警告）、step-start/step-finish → step(begin/end)、step-finish 的 tokens/cost → usage、patch → notice、compaction → notice、file → notice（**只留 mime/filename，绝不带 data URI 的 base64**，实测单条可达数 MB）。**验证**：本机 4 个 opencode 任务全部 `timestamp+prompt` high 置信度定位；对 4 个真实会话做「库内 part 数 vs 输出 unique part 数」对账全等（排除有意的空白文本 part，其计数经 `meta.emptyTextParts` 透出），强制小批量分页（25 事件/批，最多 17 批）与一次性读取的事件数完全一致，base64 零泄漏；另真跑一次 `opencode run` 做运行中采样（6 秒内 parts 11→14、events 18→22）确认近实时可读，随后用 `opencode session delete` 删除该探测会话。测试 227 项全绿
- 2026-09-20 六次增补（pi 日志适配）：`mapping.js` 新增 `findPiRef`、新增 `server/src/core/adapters/pi.js` 并注册进适配器分发表。pi 由 dim 经 `pi-acp`（ACP 桥接，`desktop-config.json` 的 `externalAgents.pi.execPath`）委托，会话落在 `~/.pi/agent/sessions/<cwd 编码>/<ISO 时间戳>_<uuid>.jsonl`（目录名 = cwd 的 `/` 换 `-` 后首尾各加 `-`，文件名前缀就是创建时间 UTC）。**与 opencode 恰好相反**：pi 是**追加式** JSONL（version 3），已落盘的行不会被改写，所以直接用**字节偏移游标**（与 kimi/codex/grok 同构，尾部半行不消费），不需要 opencode 那套稳定边界判定；代价是**回合粒度**——每个 assistant 回合写一行，回合进行中（模型正在流式输出）看不到中间内容，要等该行落盘。全量实测 87 个会话 / 22081 行 / **零坏行**，覆盖 record 类型 session / model_change / thinking_level_change / custom / message，role 四种（user 321 / assistant 9973 / toolResult 11486 / system 1），content part 四种（text / thinking / toolCall / image）。两处大字段必须剥离：`image` part 的 `data` 是 base64（实测单条 312KB，在 assistant 与 toolResult 两处都会出现）、`toolResult.details` 是 **Python repr 字符串**而非 JSON（单条可含整段文件内容）。事件映射：user → notice(user_message)、assistant 的 thinking/text/toolCall → think/text/tool_call、回合末尾 usage（含 cacheRead 与 cost）、`errorMessage` → status=error 的 notice(error)、toolResult → tool_result（`isError` 决定 status）、system → notice(system)、custom → notice(custom:<customType>)。**验证**：89 个真实会话全量读取，强制小批量分页（3 行/批）与一次性读取事件数**零差异**、**零 unknown 事件、零 warning**；image base64 指纹在事件流中出现 **0 次**（用原始 data 前 60 字符做指纹精确比对）；mapping 用真实文件名时间戳构造任务，**89/89 全部定位到自己、零误配**。测试 239 项全绿
- 2026-09-20 七次增补（日志详情页进一步压缩）：日志详情页的**默认形态改为一行轨迹摘要**——原先逐条铺开的思考行 / 工具调用行 / 工具结果行全部折进一行 `思考3轮 · 读1次文件、改2次文件、执行2次命令、搜索1次`（分类规则：`toolLabel` 的显示名映射到 read/write/exec/search/other，为 0 的类别不显示；既无 think 也无 tool_call 时兜底为「系统事件 N 条」），点击整行展开完整轨迹（rail 内逐行明细，系统事件仍聚合为一行）；**Agent 的正文输出（text 事件）保持全宽可见**，事件流按正文切分，因此「正文—工具—正文」的任务会得到多个摘要行。折叠段内有失败调用时摘要行标红（折叠态即可察觉）；展开状态以首事件 seq 为 key（`traj:<seq>`），2s 轮询重建后保持。原先的「工具调用」聚合组（≥2 个连续调用才折叠）被这一层取代，单个调用也一并折叠，`renderWorkGroup` 移除。测试 242 项全绿；浏览器实测（iframe 假宿主，14 条事件 + 一段正文）：默认态 `#events` 只有 2 个顶层节点（摘要行 + 正文）、`.ev-body` 计数为 0，点击摘要行后明细 rail 出现，再点收回
