...
| opencode | `~/.local/share/opencode/opencode.db` + `storage/` + `log/` | SQLite（表 `session` `message` `part` `event` `todo` 等；`data` 是 JSON 文本列，内容与 role 都在里面） | **日志已适配**（2026-09-20，`adapters/opencode.js`）：状态式存储——part 行会**原地改写**（tool 从 `running` → `completed`）、text/reasoning 是**流式写入**（实测 time_updated − time_created 最长 83s），故用「最后一条 step 标记（step-start/step-finish）」作**稳定边界**，只输出已结束 step 的内容；`file` part 的内联 base64 绝不入事件；定位用 `session.time_created` + 30s 窗口，消歧为 prompt 指纹 > 标题 > Issue token > delta |
| grok | `~/.grok/sessions/<urlencoded-workspace>/<uuid>/` | 多文件目录（19 项/会话）+ `session_search.sqlite` | 后续扩展 |
| pi | `~/.pi/agent/sessions/<cwd 编码>/<ISO 时间戳>_<uuid>.jsonl`（cwd 的 `/` 换 `-`、首尾各加 `-`） | **追加式 JSONL（version 3）**：首行 `session`（id/cwd/timestamp），其后 `model_change` / `thinking_level_change` / `custom` / `message`；`message.role` ∈ user / assistant / toolResult / system，content part ∈ text / thinking / toolCall / image | **日志已适配**（2026-09-20，`adapters/pi.js`）：与 kimi/grok 同构的**字节偏移游标**（追加式，无原地改写）。dim 经 `pi-acp` 委托（`desktop-config.json` 的 `externalAgents.pi.execPath`），会话落同一目录。两处必须剥离：`image` part 的 base64（实测单条 312KB）与 `toolResult.details`（**Python repr 字符串**、非 JSON，单条可含整段文件内容）。定位用文件名前缀的 UTC 时间戳，消歧为 prompt 指纹 > Issue token > delta |
